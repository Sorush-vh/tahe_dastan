                        ...
                        if(read_word(i+1,address->full_input,temp)) {
                            ...
                            if(temp[0]=='"'){
                                ...
                                while (1)
                                {
                                    ...
                                    if( temp[strlen(temp)-1]=='"' && temp[strlen(temp)-2]!=92) break;
                                    buff++;
...
//tree compare grep
    if( order_code!=14  && order_code!=10 && order_code!=13) 
...
else strcpy(address->undo_content,NO_UNDO);
}
...
void execute_command(struct orders *address,int arg_code){
    ...
    if (arg_code==11)
    {
        ...
        if(n<1){
                printf("invalid size for undoing this file\n");
                return ;
            }

        while (1)
        {
            if( !checklines(NO_UNDO,address->undo_content) ) count++;
            if (count==n)
            {
                ...
                while (1)
                {
                    if(address->next) address=address->next;
                    else break;
                }
                return ;
            }
...
int read_head(struct orders *address)
{
    if(!read_word(0,address->full_input,address->order_head)) return 0;
    else{
        if(checklines("createfile",address->order_head) ) return 1;
        else if(checklines("insertstr",address->order_head)) return 2;
        else if(checklines("cat",address->order_head)) return 3;
        else if(checklines("removestr",address->order_head)) return 4;
        else if(checklines("copystr",address->order_head)) return 5;
        else if(checklines("cutstr",address->order_head)) return 6;
        else if(checklines("pastestr",address->order_head)) return 7;
        else if(checklines("find",address->order_head)) return 8;
        else if(checklines("replace",address->order_head)) return 9;
        else if(checklines("grep",address->order_head)) return 10;
        else if(checklines("undo",address->order_head)) return 11;
        else if(checklines("auto-indent",address->order_head)) return 12;
        else if(checklines("compare",address->order_head)) return 13;
        else if(checklines("tree",address->order_head)) return 14;
        else return 0;
    }
}
...
void indent(char *file_name)
{
    ...
    while (1)
    {
        ...
        if(hold[i]=='{') {
            int ff= (hold[i-1]==' ');
            insert_str(Sinter,i+1,hold);
           if(!ff) insert_str(Sspace,i,hold);
           if(!ff) i++;
        }
        i++;
    }
